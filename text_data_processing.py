...
MAX_FILENAME_WORDS = int(os.getenv("MAX_FILENAME_WORDS", 3))
MAX_FOLDERNAME_WORDS = int(os.getenv("MAX_FOLDERNAME_WORDS", 2))


def summarize_text_content(text, text_inference):
    """Summarize the given text content."""
    # Truncate text if it's too long (leaving room for prompt)
    max_input_length = 2500  # Adjust this value as needed
...
def process_single_text_file(args, text_inference, silent=False, log_file=None):
    ...
    with Progress(
        ...
        task_id = progress.add_task(
            f"Processing {os.path.basename(file_path)}", total=1.0
        )
        foldername, filename, description = generate_text_metadata(
            text, file_path, progress, task_id, text_inference
        )
...
def process_text_files(text_tuples, text_inference, silent=False, log_file=None):
    ...
    for args in text_tuples:
        data = process_single_text_file(
            args, text_inference, silent=silent, log_file=log_file
        )
        results.append(data)
    return results
...
Filename:"""
    ...
    # Handle both string and dictionary responses
    if isinstance(filename_response, dict):
        filename = filename_response["choices"][0]["text"].strip()
    else:
        filename = str(filename_response).strip()
...
Category:"""
    ...
    # Handle both string and dictionary responses
    if isinstance(foldername_response, dict):
        foldername = foldername_response["choices"][0]["text"].strip()
    else:
        foldername = str(foldername_response).strip()
    ...
    # Process filename
    filename = clean_ai_output(filename, max_words=MAX_FILENAME_WORDS)
    if not filename or filename.lower() in ("untitled", ""):
        # Use keywords from the description
        filename = clean_ai_output(description, max_words=MAX_FILENAME_WORDS)
    if not filename:
        filename = "document_" + os.path.splitext(os.path.basename(file_path))[0]

    sanitized_filename = sanitize_filename(filename, max_words=MAX_FILENAME_WORDS)

    # Process foldername
    foldername = clean_ai_output(foldername, max_words=MAX_FOLDERNAME_WORDS)
    if not foldername or foldername.lower() in ("untitled", ""):
        # Attempt to extract keywords from the description
        foldername = clean_ai_output(description, max_words=MAX_FOLDERNAME_WORDS)
        if not foldername:
            foldername = "documents"

    sanitized_foldername = sanitize_filename(foldername, max_words=MAX_FOLDERNAME_WORDS)
